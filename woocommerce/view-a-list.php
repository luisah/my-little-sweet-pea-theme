    ...
    <div id="wl-wrapper" class="product woocommerce">
        ...
        <?php if ($wishlist_items && count($wishlist_items)) : ?>
            <?php if (isset($_GET['preview']) && $_GET['preview']) : ?>
                <div class="woocommerce-info woocommerce_info"><a href="<?php echo $wishlist->the_url_edit(); ?>" class="button">Return to your view</a> <?php _e('This is how other people will see your Wish List', 'wc_wishlist'); ?></div>
            <?php endif; ?>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" class="wl-actions-table wl-right">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php wp_dropdown_categories(array('taxonomy' => 'product_cat', 'class' => 'wl-sel', 'name' => 'wlitemcat', 'show_option_all' => 'All', 'selected' => $wlitemcat, 'hierarchical' => true, 'include' => array_keys($wishlist_item_categories))); ?>
                                </td>
                                <td>
                ...
                <tbody>
                    ...
                    if (sizeof($wishlist_items) > 0) :
                                ...
                                <tr class="cart_table_item">
                                    ...
                                    <td class="product-purchase">
                                        <?php if ($_product->is_in_stock()) : ?>
                                            <a href="<?php echo woocommerce_wishlist_url_item_add_to_cart($wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="button">Add to Cart</a>
                                        <?php endif; ?>
						...
						<tr><!-- the tds need to be individual for woothemes responsive mode, since they target the first td specifically and hide it. WIth colspan this causes issues once in responsive mode -->
							...
							<td class="product-purchase"><a href="<?php echo woocommerce_wishlist_url_add_all_to_cart($wishlist->id, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="button alt wl-add-all">Add All To Cart</a></td>
						</tr>
                    <?php endif; ?>
                </tbody>
            </table>
            ...
            <?php if ($sharing == 'Public' || $sharing == 'Shared') : ?>
				<div class="wl-share-url"><strong>Wish List URL:  </strong><?php echo $wishlist->the_url_view($sharing == 'Shared'); ?></div>
			<?php endif; ?>
			<?php if ($sharing == 'Public' || $sharing == 'Shared') : ?>
				<?php if ($wishlist_items && count($wishlist_items)) : ?>
			                <div class="wl-meta-share">
				                <div class="grid">Share This List with Friends &amp; Family!</div>
